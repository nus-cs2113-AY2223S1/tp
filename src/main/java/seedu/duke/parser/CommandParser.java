...
public class CommandParser {
    public static Command getUserCommand(String userInput) throws InvalidUserCommandException {
        ...
        if (userInputTokenized.length < 1){
            throw new InvalidUserCommandException("Error! Missing command. Please follow the command format provided!");
        }
        String userInputCommand = userInputTokenized[0];
        switch (userInputCommand){
        case "/create":
            if (!isValidCreateCommand(userInputTokenized)){
                throw new InvalidUserCommandException("Error! Invalid create command. Please follow the command format provided");
            }
        ...
        case "/add":
            if (!isValidAddCommand(userInputTokenized)){
                throw new InvalidUserCommandException("Error! Invalid add command. Please follow the command format provided");
            }
        ...
        case "/view":
            if (!isValidViewCommand(userInputTokenized)){
                throw new InvalidUserCommandException("Error! Invalid view command. Please follow the command format provided");
            }
        ...
        case "/delete":
            if (!isValidDeleteCommand(userInputTokenized)){
                throw new InvalidUserCommandException("Error! Invalid delete command. Please follow the command format provided");
            }
        ...
        default:
            throw new InvalidUserCommandException("Error! Unidentified command. Please follow the command format provided!");
        }
    }

    private static boolean isValidViewCommand(String[] parameters){
        if (!parameters[1].startsWith("u/") && !parameters[1].startsWith("ALL") && !parameters[1].startsWith("MODULES") || parameters.length > 2){
            return false;
        } else {
            return true;
        }
